...
def pointInTriangle(p0, p1, p2, checked_point : tuple[int,int]) -> bool:
...
def anyPointInTriangle(p0, p1, p2, points : list) -> bool:
    '''Check if ANY point in points is inside triangle'''
    for p_check in points:
        if p_check != p0 and p_check != p1 and p_check != p2 and pointInTriangle(p0,p1,p2,p_check):
            return True
    return False

def checkEar(idx, points : list) -> bool:
    '''Check if index idx is an ear'''
    n = len(points)
    t = [points[(idx-1)%n], points[idx], points[(idx+1)%n]]
    return signal(t[1],t[2],t[0]) > 0 and not anyPointInTriangle(*t, points)

def earclip(points : list):
    '''Triangulate a polygon using ear clipping method, given a list of (x,y) points'''
    ears = {i for i in range(len(points)) if checkEar(i, points)}
    print(ears)
